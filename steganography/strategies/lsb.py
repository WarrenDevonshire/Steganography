...
def hide_data_in_LSB(pixels, data, seed):
    """
    Hide data in carry and return a numpy array.
    :param pixels: an np array image to hide data in
    ...
    for index, bit in enumerate(size_bytes):
        ...
        bit_mask = 1
        byte = pixels[random_index] & ~bit_mask
        if bit:
            byte = byte | bit_mask
        pixels[random_index] = byte
...
def get_data_in_LSB(pixels, seed):
    """
    Get data from carry and return a numpy array of bytes.
    :param pixels: an np array image
    :param seed: an integer
    ...
    for index, random_index in enumerate(seq[SIZE_LENGTH:(data_size + SIZE_LENGTH)]):
        data_bits[index] = pixels[random_index] % 2
